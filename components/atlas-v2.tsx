...
import { Activity, Compass, Database, Globe2, Home, Layers3, Minus, Plus, Route, Settings2, Share2, X } from 'lucide-react';
...
const MobilityDrawer = dynamic(() => import('@/components/mobility-drawer'), { ssr: false });
...
export default function AtlasV2() {
  ...
  const [panelOpen, setPanelOpen] = useState(false), [panelTab, setPanelTab] = useState<PanelTab>('intel'), [toolsOpen, setToolsOpen] = useState(false), [mobilityOpen, setMobilityOpen] = useState(false), [shareMessage, setShareMessage] = useState('');
  ...
  const liveSelectedTraffic = selectedTraffic ? trafficTargets.find(target => target.id === selectedTraffic.id) ?? selectedTraffic : null;
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (mobilityOpen) { setMobilityOpen(false); return; }
      if (toolsOpen) { setToolsOpen(false); return; }
      if (panelOpen) setPanelOpen(false);
    };
    window.addEventListener('keydown', onKeyDown);
    return () => window.removeEventListener('keydown', onKeyDown);
  }, [panelOpen, toolsOpen, mobilityOpen, selectedSignal, selectedTraffic]);
  ...
  return <main className={`earth-app ${styles.app}`}>
    ...
    <header className={styles.header}>
      ...
      <div className={styles.headerActions}><button title="Intelligence" aria-expanded={panelOpen && panelTab==='intel'} onClick={() => { setToolsOpen(false); setMobilityOpen(false); setPanelOpen(open => panelTab === 'intel' ? !open : true); setPanelTab('intel'); }}><span className={styles.liveDot}/><Database size={17}/><span>Intelligence</span></button><button title="Mobility Center" aria-expanded={mobilityOpen} onClick={() => { setPanelOpen(false); setToolsOpen(false); setMobilityOpen(open => !open); }}><Route size={17}/><span>Mobility</span></button><button title="Copy scene link" onClick={() => void share()}><Share2 size={17}/><span>Share</span></button><button title="Tools" aria-expanded={toolsOpen} onClick={() => { setPanelOpen(false); setMobilityOpen(false); setToolsOpen(open => !open); }}><Settings2 size={17}/><span>Tools</span></button></div>
    </header>
    <nav className={styles.rail} aria-label="Globe controls"><button title="Home" aria-label="Home" onClick={() => globeRef.current?.home()}><Home size={18}/></button><button title="Zoom in" aria-label="Zoom in" onClick={() => globeRef.current?.zoom(1)}><Plus size={18}/></button><button title="Zoom out" aria-label="Zoom out" onClick={() => globeRef.current?.zoom(-1)}><Minus size={18}/></button><button title="Face north" aria-label="Face north" onClick={() => globeRef.current?.north()}><Compass size={18}/></button><button title="Map layers" aria-label="Layers" aria-expanded={panelOpen && panelTab==='map'} onClick={() => { setToolsOpen(false); setMobilityOpen(false); setPanelOpen(open => panelTab === 'map' ? !open : true); setPanelTab('map'); }}><Layers3 size={18}/></button></nav>
