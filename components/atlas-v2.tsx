...
export default function AtlasV2() {
  ...
  useEffect(() => {
    ...
    const latest = trafficTargets.find(target => target.id === selectedTraffic.id);
    if (latest && latest.observedAt !== selectedTraffic.observedAt) setSelectedTraffic(latest);
  }, [trafficTargets, selectedTraffic]);
